...
function Home() {
    ...
    //ページが表示された時に1度だけ実行される初期設定処理
    //Minaブロックチェーンと接続するための準備をする
    //ユーザーのウォレットを確認し、そのウォレットがブロックチェーン上で有効かを確認する
    //zkAppの状態を取得し、その状態を画面に表示できるようにする
    (0,react.useEffect)(()=>{
        //指定した秒数だけ処理を一時停止させるための非同期関数
        //Promiseは非同期処理の状態を管理するためのオブジェクト
        //voidは、このPromiseが特に値を返さないことを示す
        ...
        async ()=>{
            if (!state.hasBeenSetup) {
                //ユーザーに「web workerを読み込んでいる」ことを表示するための処理
                setDisplayText("Loading web worker...");
                console.log("Loading web worker...");
                //Minaブロックチェーンと通信するためのクライアント
                const zkappWorkerClient = new ZkappWorkerClient();
                //web workerを完全にロードするまでの待ち時間
                await timeout(5);
                setDisplayText("Done loading web worker...!");
                console.log("Done loading web worker...");
                //MinaがDevnetに接続するように設定
                await zkappWorkerClient.setActiveInstanceToDevnet();
                //Auro Walletがインストールされているかを確認
                //ウォレットが見つからない場合、hasWalletという状態をfalseにして処理を終了
                ...
                //Minaブロックチェーン上に、ウォレットに対応するアカウントが存在するかどうかを確認する
                //存在しない場合、エラーが返される
                ...
                //zkAppの公開鍵を使用して、ブロックチェーン上にあるスマコンの状態を取得
    ...
    //ウォレットアカウント(fee payer account)がブロックチェーン上に存在するかどうかを定期的に確認する
    //ウォレットアカウントが存在するまで待機し、存在が確認できたら次の処理に進むという流れ
    (0,react.useEffect)(()=>{
        async ()=>{
            if (state.hasBeenSetup && !state.accountExists) {
                for(;;){
                    ...
                    //アカウントが存在するかを5秒ごとに再確認
                    await new Promise((resolve)=>setTimeout(resolve, 50000));
                }
                //アカウントが見つかると、accountExistsの値をtrueに変更し、Reactの状態が更新されてコンポーネントが再レンダリング
                setState({
                    ...state,
                    accountExists: true
                });
    ...
    //Minaブロックチェーンに対してトランザクションを作成し、送信する処理を行う
    //ユーザーがボタンを押した時にトランザクションが作成され、Minaネットワークに送信される
    const onSendTransaction = async ()=>{
        //トランザクションを作成中であることを示すcreatingTransactionの値をtrueに変更
        setState({
            ...state,
        ...
        //ブロックチェーン上にウォレットに対応するアカウントを確認するための処理
        //結果が帰るまで次の処理に進まずに待機
        await state.zkappWorkerClient.fetchAccount({
            publicKey: state.publicKey
        });
        //zkAppに対してトランザクションを生成するための処理
        await state.zkappWorkerClient.createUpdateTransaction();
        setDisplayText("Creating proof...");
        console.log("Creating proof...");
        //トランザクションのゼロ知識証明を生成する処理
        //Minaでは、トランザクションを送信する前に証明を作成する必要がある
        await state.zkappWorkerClient.proveUpdateTransaction();
        console.log("Requesting send transaction...");
        setDisplayText("Requesting send transaction...");
        //トランザクションデータをJSON形式で取得
        const transactionJSON = await state.zkappWorkerClient.getTransactionJSON();
        setDisplayText("Getting transaction JSON...");
        console.log("Getting transaction JSON...");
        //Minaブロックチェーンにトランザクションを送信し、結果として返ってくるトランザクションIDを取得する
        //
        const { hash } = await window.mina.sendTransaction({
            //引数1:トランザクションの内容をJSON形式で表したもの
            transaction: transactionJSON,
            //引数2:Minaネットワーク上でfeeを支払うアカウントの情報
            feePayer: {
                fee: transactionFee,
                memo: ""
            }
        ...
        //トランザクションのリンクを状態として保存=>setDiplayTextで表示
    ...
    //Minaブロックチェーンに接続してzkAppの最新の状態を取得し、その状態をReactのUIに反映する
    const onRefreshCurrentNum = async ()=>{
        console.log("Getting zkApp state...");
        setDisplayText("Getting zkApp state...");
        //Minaブロックチェーン上で指定されたzkAppのアカウント情報を取得
        await state.zkappWorkerClient.fetchAccount({
            //zkAppの公開鍵を使ってそのアカウントを探す
    ...
    //Create UI elements
    //Walletがブラウザにインストールされているかを確認
    //インストールされていない場合は、Auro Walletのインストールリンクを表示
    let hasWallet;
    ...
    //トランザクションの表示
    //トランザクションの詳細があればリンク表示、なければ進行状況やメッセージ表示
    //transactionlinkは、useStateフックで定義されている状態変数
    ...
    //初期設定が完了したが、Minaネットワーク上にアカウントが存在しない場合
    if (state.hasBeenSetup && !state.accountExists) {
